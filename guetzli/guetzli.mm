...
int test2(const char* inputPath,const char* outputPath){
#ifdef __USE_GPERFTOOLS__
    ...
    WriteFileOrDie(outputPath, out_data);
#ifdef __USE_GPERFTOOLS__
    ProfilerStop();
#endif
    return 0;
...
int test(const char* inputPath,const char* outputPath){
#ifdef __USE_GPERFTOOLS__
    ProfilerStart("guetzli.prof");
#endif
    std::set_terminate(TerminateHandler);
...
#endif
//#ifdef __APPLE__
//    else if (!strcmp(argv[opt_idx], "--metal"))
//    {
//        g_mathMode = MODE_METAL;
//    }
//#endif
