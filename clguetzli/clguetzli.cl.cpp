...
namespace guetzli
{
    ...
    void ButteraugliComparatorEx::Compare(const OutputImage& img)
    {
        ...
        else if (MODE_OPENCL == g_mathMode)
        {
            ...
            args_d_t &= getOcl();
            ometal_channels xyb0 = allocMemChannels(channel_size, rgb_orig_opsin[0].data(), rgb_orig_opsin[1].data(), rgb_orig_opsin[2].data());
            ometal_channels xyb1 = allocMemChannels(channel_size, rgb1[0].data(), rgb1[1].data(), rgb1[2].data());
            
            cl_mem mem_result = allocMem(channel_size);
            
            clOpsinDynamicsImageEx(xyb1, xsize, ysize);
            clDiffmapOpsinDynamicsImageEx(mem_result, xyb0, xyb1, xsize, ysize, comparator_.step());
            
            cl_int err = clEnqueueReadBuffer(commandQueue, mem_result, false, 0, channel_size, distmap_.data(), 0, NULL, NULL);
            LOG_CL_RESULT(err);
            err = clFinish(commandQueue);
            LOG_CL_RESULT(err);
            
            clReleaseMemObject(mem_result);
            releaseMemChannels(xyb0);
            releaseMemChannels(xyb1);
        ...
        else if (MODE_METAL == g_mathMode)
        {
            ...
            metal_mem mem_result = allocMem(channel_size,NULL);
            ometal_channels *xyb0 = allocMemChannels(channel_size, rgb_orig_opsin[0].data(), rgb_orig_opsin[1].data(), rgb_orig_opsin[2].data());
            //NSData* data1 = [NSData dataWithBytesNoCopy:[xyb0->r contents ] length: [xyb0->r length] freeWhenDone:false ];
            ometal_channels *xyb1 = allocMemChannels(channel_size, rgb1[0].data(), rgb1[1].data(), rgb1[2].data());
            metalOpsinDynamicsImageEx(xyb1, xsize, ysize);
            metalDiffmapOpsinDynamicsImageEx(mem_result, xyb0, xyb1, xsize, ysize, comparator_.step());
            
            metalEnqueueReadBuffer(m_ometal.commandQueue, mem_result, false, 0, channel_size, distmap_.data(), 0, NULL, NULL);
