...
#if defined(__USE_OPENCL__)||(__USE_METAL__)
...
#ifdef __USE_METAL__
#import  "clguetzli/clguetzli.cl.h"
#endif
...
// IntFloatPair «Œ™¡Àƒ£ƒ‚output_order input_orderµƒvector
...
// batch «÷∏“—æ≠∂˛Œ¨øÈ’πø™Œ™¡À“ªŒ¨øÈ
__kernel void clComputeBlockZeroingOrderEx(
                                           __global const coeff_t *orig_batch_0,       // ‘≠ ºÕºœÒœµ ˝
                                           __global const coeff_t *orig_batch_1,       // ‘≠ ºÕºœÒœµ ˝
                                           __global const coeff_t *orig_batch_2,       // ‘≠ ºÕºœÒœµ ˝
                                           __global const float   *orig_image_batch,   // ‘≠ ºÕºœÒpregamma
                                           __global const float   *mask_scale,         // ‘≠ ºÕºœÒµƒƒ≥∏ˆ…Ò√ÿ≤Œ ˝
                                           const int              block_xsize,
                                           const int              block_ysize,
                                           const int              image_width,
                                           const int              image_height,
                                           
                                           __global const coeff_t *mayout_batch_0,     //  ‰≥ˆ±∏—°Õºµƒœµ ˝
                                           __global const coeff_t *mayout_batch_1,     //  ‰≥ˆ±∏—°Õºµƒœµ ˝
                                           __global const coeff_t *mayout_batch_2,     //  ‰≥ˆ±∏—°Õºµƒœµ ˝
                                           ...
                                           const int factor,                                 // µ±«∞≤Œ”Î‘ÀÀ„µƒfactor
                                           const int comp_mask,                              // µ±«∞≤Œ”Î‘ÀÀ„µƒchannel
                                           ...
                                           __global CoeffData *output_order_list/*out*/)
{
    ...
    int block_idx = 0;        // ∏˘æ›œ¬√Êmask√¸÷–µƒchannel¿¥º∆À„indx
    ...
    while (input_order.size > 0)
    {
        ...
        {   // err∂”¡– «÷Ω•‘ˆ¥Ûµƒ£¨»Áπ˚’‚¿Ô“—æ≠≥¨π˝ErrorLimit£¨∫Û–¯µƒº∆À„æÕ «»ﬂ”‡µƒ¡À
            break;
        }
    ...
    // ◊¢“‚output_order’‚¿ÔµƒresizeæÕ «∞—Œ≤≤øµƒ÷√Œª0
    float min_err = 1e10;
...
{
    // ≤ŒøºclBlurEx2µƒ µœ÷£¨sigma = 1.1£¨’‚ ±step°¢diff∂ºΩ´ÃÿªØŒ™πÃ∂®÷µ
    ...
    float expn[5] = { (float)exp(scaler * (-diff) * (-diff)),
        (float)exp(scaler * (-diff + 1) * (-diff + 1)),
        (float)exp(scaler * (-diff + 2) * (-diff + 2)),
        (float) exp(scaler * (-diff + 3) * (-diff + 3)),
        (float) exp(scaler * (-diff + 4) * (-diff + 4))};
